...
public class ServerSendFile {
	public static void sendFileRequest(File file, InetAddress serverAddress, int serverPort) {
		try (DatagramSocket socket = new DatagramSocket()) {
			...
			DatagramPacket fileNameSizePacket = new DatagramPacket(fileNameSizeData, fileNameSizeData.length,
					serverAddress, serverPort);
			socket.send(fileNameSizePacket);

			// Gửi tên file
			byte[] fileNameData = fileName.getBytes();
			DatagramPacket fileNamePacket = new DatagramPacket(fileNameData, fileNameData.length, serverAddress,
					serverPort);
			...
			DatagramPacket fileSizePacket = new DatagramPacket(fileSizeData, fileSizeData.length, serverAddress,
					serverPort);
